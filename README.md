...
- **多模型竞速**：同一题可以同时丢给多个模型。
- **自动拉题**：协调器会从 CTFd 自动发现新题并启动 swarm。
- **自动停题**：一旦题目在 CTFd 上被确认已解，对应 swarm 会自动结束。
...
| 路径 | 作用 |
|------|------|
| [backend/cli.py](backend/cli.py) | CLI 入口，定义了 `ctf-solve` 和 `ctf-msg` |
...
单题模式不会自动去 CTFd 搜索并下载这道题；你需要先准备好本地目录。
...
5. 用 [backend/poller.py](backend/poller.py) 每 5 秒轮询一次 CTFd。
6. 对每道未解题调用 [backend/agents/coordinator_core.py](backend/agents/coordinator_core.py) 的 `do_spawn_swarm()`。
7. 如果本地没有对应题目目录，就先通过 [backend/ctfd.py](backend/ctfd.py) 自动拉题。
