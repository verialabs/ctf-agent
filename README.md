...
当前实现仍然保留 `poller -> coordinator_loop -> swarm -> solver` 的主骨架，但控制面已经显式化为可审计、可替换的模块：

- `Platform State`：平台事实（题单/已解/题面元信息），由 `poller + platform client` 提供
- `Runtime State`：运行态快照（`CompetitionState / ChallengeState / SwarmState`），从平台事实 + swarm/结果/成本 汇总得到
- `Policy Engine`：规则优先的调度决策；当前稳定接线的是 spawn/broadcast，并负责把 advisor suggestion 过滤/转换为可执行 action
- `Working Memory`：单题短期记忆（从 solver trace 增量提炼）
- `Knowledge Store`：跨题可复用知识（从 working memory 晋升 + 命中与已应用保护）
- `Advisor adapters`：provider-specific 推理适配器（Azure/Claude/Codex），只产出“建议”，不直接承载整场控制逻辑
- `Solver Runtime / Swarms`：每题一个 swarm，并行驱动多模型 solver 容器执行工具链

同时，共享控制面已经接入 `coordinator_loop` 的 `policy_engine -> actions -> coordinator_core` 路径；但 provider-specific coordinator 的 legacy 直控协调路径仍然存在，当前更准确的状态是“共享控制面与 provider 直控路径并存”，而不是已经完全退化成纯 advisor 壳层。
...
flowchart TB
    ...
    subgraph Control["控制层（共享 coordinator loop + 显式控制面）"]
        Poller["Poller<br/>每 5 秒同步题单/已解/题目详情"]:::control
        PState["Platform State<br/>平台事实（题单/已解/元信息）"]:::control
        RState["Runtime State<br/>CompetitionState / ChallengeState / SwarmState"]:::control
        WM["Working Memory<br/>单题短期记忆（trace 提炼）"]:::control
        KS["Knowledge Store<br/>跨题知识（晋升/匹配/已应用保护）"]:::control
        PE["Policy Engine<br/>规则优先决策 -> Action 列表"]:::control
        AE["Action Executor<br/>coordinator_core 执行动作"]:::control
        Advisors["Advisor adapters<br/>azure / claude / codex"]:::control
    end

    subgraph Swarms["题目求解层（Solver Runtime / Swarms）"]
        S1["Swarm<br/>每题一个 swarm"]:::swarm
        Solvers["Solvers<br/>多模型并行竞速"]:::swarm
    end

    subgraph Sandboxes["隔离执行层"]
        Boxes["Solver Containers<br/>pwntools / r2 / gdb / python"]:::sandbox
    end
    ...
    Solvers --> Boxes
    Solvers -.执行轨迹.-> Logs
    Solvers -.题解草稿.-> Writeups
```

整体闭环：

1. `Poller` 持续同步平台事实（题单/已解/题面），形成 `Platform State`。
2. 共享 `coordinator_loop` 汇总平台事实、当前 swarm/结果/成本，构建 `Runtime State` 快照，并增量提炼 `Working Memory`、晋升/匹配 `Knowledge Store`。
3. `Policy Engine` 基于 `Runtime State + Memory + Knowledge` 产出结构化动作列表，并由 `coordinator_core` 执行。
4. 每道未解题由 `Swarm` 并行驱动多模型 `Solver Runtime`；solver 在容器中执行工具链，trace 回流到记忆与策略闭环。平台确认提交正确后，系统会自动收尾对应 swarm，并按配置决定是否释放环境、生成 writeup、退出整场。
